...
// --- 6. process memory sampling + budget -------------------------------------

/** Default dsh memory budget (bytes). User target: ~500 MB. */
export const DEFAULT_MEMORY_BUDGET = 500 * 1024 * 1024;

/**
 * Sample the current process memory (DI wrapper around process.memoryUsage).
 *
 * @param {{memoryUsage?: Function}} [opts] injected sampler for tests
 * @returns {{rss: number, heapUsed: number, heapTotal: number, external: number, arrayBuffers: number}}
 */
export async function sampleMemory({ memoryUsage } = {}) {
...
/**
 * Where the process RSS sits against the memory budget (pure function).
 *
 * @param {{memory?: object, budget?: number}} [opts]
 *   memory: sampleMemory() result (needs .rss); budget defaults to 500 MiB
 * @returns {{rss: number, budget: number, ratio: number, remaining: number, over: boolean, pct: number}}
 */
export function memoryBudget({ memory = {}, budget = DEFAULT_MEMORY_BUDGET } = {}) {
  const rss = Number.isFinite(memory?.rss) ? memory.rss : 0;
  const b = Number.isFinite(budget) && budget > 0 ? budget : DEFAULT_MEMORY_BUDGET;
  const ratio = b > 0 ? rss / b : 0;
  return {
    rss,
    budget: b,
    ratio,
    remaining: Math.max(0, b - rss),
    over: rss > b,
