...
//
// Pulls three core network sources:
...
//   4. dsh.so verification index over HTTPS (best-effort only; failure ignored)
...
//   5. marketplace.json — the full dsh ecosystem snapshot (ownerRepo as
//      items[].name, displayName, numeric dsh.so verification levels)
//   6. dshso-index.json — authoritative dsh.so verification/security verdicts
...
/**
 * Refresh the index from the three core network sources (GitHub / npm /
 * awesome), the best-effort dsh.so network index and the two local import
 * sources under <DSH_HOME>/plugin-manager-cache. The local sources never
 * fail and are re-imported on every refresh. When every network source
 * fails but the local marketplace contributed entries, the refresh still
 * succeeds — offline-first, the anonymous GitHub rate limit cannot starve
 * the index (the network error is still recorded in lastError).
 *
