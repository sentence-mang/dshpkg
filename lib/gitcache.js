...
export async function ensureGitCache(repoUrl, runner = defaultGitRunner) {
  ...
  // Defense in depth: a leading dash would be parsed as a git option (e.g.
  // "-u" for --upload-pack), not as the repository argument.
  if (url.startsWith("-")) {
    throw new Error("非法的 git 仓库地址（不能以 - 开头）");
  }
  const dir = resolveGitCache(url);
  ...
  // "--" ends git option parsing so a repository url starting with "-" can
  // never be mistaken for a flag even if it slips past the guard above.
  const clone = await runner(["clone", "--depth", "1", "--", url, dir], {});
  if (runFailed(clone)) {
 ...
 * the trigger for the SSH switching hint. Covers git's own connect errors
 * AND pnpm's git resolution error code (ERR_PNPM_GIT_RESOLVE_FAILED).
 */
export function isGitNetworkError(text) {
  ...
  return (
    ...
    t.includes("err_pnpm_git_resolve_failed")
  );
}
