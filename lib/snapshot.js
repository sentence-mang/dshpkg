...
import { statePath, listSnapshots, withSyncLock } from "./state.js";

export { listSnapshots };

/** The three manifest files that fully describe a profile's plugin set. */
export const SNAPSHOT_FILES = ["package.json", "cordis.patch.yml", "pnpm-lock.yaml"];
...
export async function restoreSnapshot(profileDir, ts) {
  // R19: a restore rewrites the same profile surface the CLI / watchdog edit
  // — serialize it under the sync lock.
  return withSyncLock(() => restoreSnapshotImpl(profileDir, ts));
}

async function restoreSnapshotImpl(profileDir, ts) {
