...
export function extractAllowBuildsKeys(text, fallbackName) {
  ...
  // 3. tokens on any line mentioning allowBuilds
  ...
  const out = [...keys];
  if (out.length === 0 && fallbackName) out.push(fallbackName);
  return out;
}
...
export function mergeAllowBuildsKeys(text, keys) {
  ...
  // Block form: insert the missing keys right after the allowBuilds line.
  const indent = (lines[idx].match(/^(\s*)/) || ["", ""])[1];
  lines.splice(idx + 1, 0, ...missing.map((k) => `${indent}  - ${k}`));
  return lines.join("\n");
}
...
async function runBuildSteps(name, build, profile, execBuild) {
  ...
  for (const command of build.commands) {
    const res = execBuild({ command, cwd: base });
...
export async function install(
  ...
  // unchanged — pnpm resolves those natively.
  const resolved = [];
  for (const entry of entries) {
    ...
    if (!dryRun) {
      try {
        ...
        const hint = isGitNetworkError(detail) ? `（${SSH_HINT}）` : "";
        return { ok: false, error: `git 仓库拉取失败: ${entry.name} ${hint}（${detail}）`, rolledBack: false };
      }
    }
    ...
    if (!dryRun && git.subdir && !existsSync(target)) {
      return {
        ok: false,
        error: `git 仓库子目录不存在: ${entry.name}（#path:${git.subdir} 在缓存中未找到，请检查配方）`,
        rolledBack: false,
      };
    }
    resolved.push({ ...entry, git, spec: `link:${target}` });
  }
