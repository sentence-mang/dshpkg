...
//
// All shelling goes through an injected runner: child_process.spawnSync on
// $DSH_BIN (default "dsh") with the inherited environment. dryRun prints the
// planned commands (Chinese, user-facing) and never invokes the runner.
// Tests inject a fake runner and never execute a real dsh/pnpm or touch a
// real profile.
...
/**
 * Default runner: spawn dsh synchronously, inheriting the environment.
 * The binary is resolved as $DSH_BIN first, then "dsh" on PATH.
 *
...
export function defaultRunner(args) {
  ...
  return spawnSync(bin, args, { env: process.env, stdio: "inherit" });
}
...
function resolveEntries(specOrRecipe, installedRecipes = {}) {
  ...
  const visit = (node) => {
    ...
    stack.pop();
    out.push({ name, spec: node.source || node.name });
  };
