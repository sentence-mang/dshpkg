...
import { composeBundleOrder } from "./governor.js";
...
export async function install(
  ...
  // 4.5 bundle load-order orchestration (best-effort, self-rolling-back):
  // guard/base layer first + dependency topological sort, so the more
  // plugins installed the more stable the load order gets (fixes the
  // "new bundle always appended to the end" load-order crash root cause).
  // A reorder failure never fails the install itself — the plugin is
  // already installed and the original order composes; the error is
  // surfaced as `reorderError`.
  let reorderError = null;
  if (!dryRun) {
    const rr = await reorderBundles(profile, { runner });
    if (!rr.ok) reorderError = rr.error ?? "bundle 重排失败";
  }
  ...
  return {
    ...
    ...(usedCache !== null ? { usedCache } : {}),
    ...(reorderError ? { reorderError } : {}),
  };
}

/**
 * Best-effort bundle load-order orchestration after a successful install:
 * guard/base layer first, then dependency topological sort (lib/governor.js
 * composeBundleOrder). Reversible: if the re-ordered tree fails to compose
 * (dump-config), the original order is restored. Never throws — returns
 * {ok, changed, error?}.
 *
 * @param {string} profile profile name
 * @param {{runner?: Function}} [opts] injectable dsh runner (default defaultRunner)
 * @returns {Promise<{ok: boolean, changed: boolean, error?: string, cycles?: string[], missing?: string[]}>}
 */
export async function reorderBundles(profile, { runner = defaultRunner } = {}) {
  try {
    const profileDir = await resolveProfileDir(profile);
    if (!profileDir) return { ok: true, changed: false };
    ...
    const bundles = Array.isArray(manifest?.dsh?.profile?.bundles)
      ...
      : [];
    if (bundles.length < 2) return { ok: true, changed: false };

    // per-bundle declared dependencies (read from the installed copy).
    // npm uses object form {name: version}; tolerate array form defensively.
    const deps = {};
    for (const name of bundles) {
      const own = await readJson(join(profileDir, "node_modules", name, "package.json"), null);
      const rawDeps = own?.dependencies;
      let depList = [];
      if (Array.isArray(rawDeps)) {
        depList = rawDeps.filter((d) => typeof d === "string");
      } else if (rawDeps && typeof rawDeps === "object") {
        depList = Object.keys(rawDeps);
      }
      deps[name] = depList;
    }

    const { ordered, cycles, missing } = composeBundleOrder({ bundles, deps });
    const same =
      ordered.length === bundles.length && ordered.every((n, i) => n === bundles[i]);
    if (same) return { ok: true, changed: false };

    const writeBundles = async (list) => {
      const target = manifest.dsh ?? {};
      target.profile = target.profile ?? {};
      target.profile.bundles = list;
      manifest.dsh = target;
      await writeFile(
    ...
    if (!res || res.status !== 0) {
      ...
      return {
        ok: false,
        changed: false,
        error: `bundle 重排冒烟失败，已恢复原顺序（${failDetail(res)}）`,
      };
    }
    return { ok: true, changed: true, cycles, missing };
  } catch (err) {
