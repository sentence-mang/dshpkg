...
/** Max missing deps actively filled per transaction. */
const MAX_FILL_DEPS = 8;

/**
 * Actively verify that the installed target's declared dependencies really
 * EXIST on the installed face (the profile's node_modules) — dshpkg does
 * not trust the installer's bookkeeping. Missing deps are actively
 * installed one by one through the official channel (a single failure does
 * not stop the others); deps still missing afterwards yield a hard Chinese
 * error so the caller rolls the transaction back. An unreadable target
 * manifest skips the verification with a warning. Never throws.
 *
 * @param {string} profileDir absolute profile directory
 * @param {string} name installed target package name
 * @param {string} profile profile name (for the fill add commands)
 * @param {Function} addRunner dsh runner for the fill add steps
 * @returns {Promise<{ok: boolean, filled: string[], error?: string}>}
 */
async function verifyAndFillDeps(profileDir, name, profile, addRunner) {
  const filled = [];
  let pkgDir;
  try {
    pkgDir = await realpath(join(profileDir, "node_modules", ...name.split("/")));
  } catch {
    console.log(`警告: 无法读取已安装清单 ${name}，跳过依赖完整性验证`);
    return { ok: true, filled };
  }
  const targetManifest = await readJson(join(pkgDir, "package.json"), null);
  const deps = targetManifest?.dependencies;
  if (!deps || typeof deps !== "object") return { ok: true, filled };

  const depExists = (depName) =>
    existsSync(join(profileDir, "node_modules", ...depName.split("/")));
  const missing = Object.keys(deps).filter((depName) => !depExists(depName));
  if (missing.length === 0) return { ok: true, filled };

  const toFill = missing.slice(0, MAX_FILL_DEPS);
  for (const depName of toFill) {
    console.log(`依赖缺失: ${name} 需要 ${depName}，主动安装中...`);
    const res = addRunner(["plugin", "--profile", profile, "add", depName]);
    if (!res || res.status !== 0) {
      console.log(`补装失败: ${depName}（${failDetail(res)}）`);
    } else {
      filled.push(depName);
    }
  }
  const stillMissing = missing.filter((depName) => !depExists(depName));
  if (stillMissing.length > 0) {
    return {
      ok: false,
      filled,
      error: `依赖不完整: ${name} 缺少 ${stillMissing.join(", ")}（已尝试补装仍失败）`,
    };
  }
  return { ok: true, filled };
}
...
/**
 * Classify a pnpm version/dependency-resolution failure so the CLI can
 * answer with actionable Chinese diagnostics instead of the raw English
 * dump. Returns a category or null (not a recognized conflict):
 *   - "no-matching-version": the requested version does not exist;
 *   - "peer-conflict":       conflicting peer dependency requirements;
 *   - "resolution-conflict": the dependency tree cannot be resolved.
 */
export function classifyVersionConflict(text) {
  const t = String(text ?? "");
  if (/ERR_PNPM_NO_MATCHING_VERSION|No matching version/i.test(t)) {
    return "no-matching-version";
  }
  if (/ERR_PNPM_PEER_DEP_ISSUE|Conflicting peer dependency|peer dependency.*conflict/i.test(t)) {
    return "peer-conflict";
  }
  if (/ERR_PNPM_DEP_RESOLUTION|cannot resolve dependency|unable to resolve/i.test(t)) {
    return "resolution-conflict";
  }
  return null;
}

/** Actionable Chinese diagnostics for a classified version conflict. */
export function versionConflictMessage(kind, name, label) {
  if (kind === "no-matching-version") {
    return `${label}: ${name}（指定的版本不存在。建议: dshpkg info ${name} 查看可用版本，或放宽版本范围，或 dshpkg hold 保持现状）`;
  }
  if (kind === "peer-conflict") {
    return `${label}: ${name}（对等依赖版本冲突。建议: dshpkg why ${name} 查看哪些插件依赖它，升级或 hold 冲突方）`;
  }
  if (kind === "resolution-conflict") {
    return `${label}: ${name}（依赖树无法解析。建议: 检查配方的依赖声明，或先移除冲突插件后重试）`;
  }
  return null;
}
 ...
 *
 * After the final smoke passes, the profile's dsh.profile.bundles list is
 * re-layered (kernel -> guardians -> stable topology, lib/order-bundles.js)
 * because the official reconciler only APPENDS new bundles at the end. A
 * re-layered config that fails the dump-config check is rolled back to the
 * pre-reorder manifest text; the install itself still succeeds (the
 * kernel-composed order is what pnpm left behind). opts.skipReorder opts
 * out entirely (tests, exotic profiles).
 *
 ...
 * @param {boolean} [opts.skipReorder=false] skip the post-install bundles re-layering
...
export async function install(
  ...
  const addStep = async (entry, { isDep }) => {
    ...
    const conflict = classifyVersionConflict(output);
    if (conflict) {
      return {
        ok: false,
        error: versionConflictMessage(conflict, entry.name, label),
      };
    }
  ...
  // 3.25 active dependency verification: the installed target's declared
  // deps must exist on the installed face; missing ones are actively
  // filled through the official channel, and deps that stay missing fail
  // the whole transaction (rolled back like any other step failure).
  if (dryRun) {
    console.log("[dry-run] 安装后验证依赖完整性（缺失将主动补装）");
  } else {
    const checkDir = await resolveProfileDir(profile);
    if (checkDir) {
      const depCheck = await verifyAndFillDeps(checkDir, self.name, profile, addRunner);
      for (const filledName of depCheck.filled) {
        rollbackNames.push(filledName);
        installedNames.push(filledName);
      }
      if (!depCheck.ok) return fail(depCheck.error);
    }
  }
  ...
  // 5. re-layer dsh.profile.bundles (kernel -> guardians -> topology). The
  // official reconciler appended the new bundles at the END; the package
  // manager contract puts guardians and dependencies where they belong.
  // Best-effort on purpose: a re-layering failure (or a re-layered config
  // that stops composing) restores the previous manifest and keeps the
  // install result — it never rolls the packages back.
  if (dryRun) {
    console.log("[dry-run] 将重排 dsh.profile.bundles: 内核层 → 守护层 → 依赖拓扑序");
  } else if (!skipReorder) {
    const profileDir = await resolveProfileDir(profile);
    if (profileDir) {
      try {
        const manifestPath = join(profileDir, "package.json");
        const before = await readTextOrEmpty(manifestPath);
        const reordered = await reorderProfileBundles(profileDir);
        if (reordered.changed) {
          const res = runner(["--profile", profile, "--dump-config"]);
          if (!res || res.status !== 0) {
            await writeFile(manifestPath, before, "utf8");
            console.log(
              "警告: bundles 自动重排后配置合成失败，已回退到重排前顺序（安装仍然成功）",
            );
          } else {
            console.log(
              `已重排 dsh.profile.bundles（${reordered.order.length} 个插件: 内核 → 守护 → 依赖拓扑序）`,
            );
          }
        }
      } catch (err) {
        console.log(
          `警告: bundles 重排失败，保留官方顺序（${String(err?.message ?? err)}）`,
        );
      }
    }
  }
