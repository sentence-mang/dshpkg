    ...
    return entries
      .filter(
        (entry) =>
          ...
          !entry.name.includes(".corrupt-"), // quarantined (R19 doctor --fix)
      )
...
/**
 * Run fn under the exclusive sync lock (R19). Contention degrades instead of
 * blocking: one retry after a short pause, then a `lock-busy` incident is
 * recorded and fn still runs UNLOCKED — every write in this codebase is an
 * atomic same-directory tmp+rename, so the worst unlocked outcome is a lost
 * update, never a torn file. Never throws because of the lock itself.
 * acquire/release/sleep are injectable for tests.
 */
export async function withSyncLock(fn, deps = {}) {
  const acquire = deps.acquireImpl ?? acquireSyncLock;
  const release = deps.releaseImpl ?? releaseSyncLock;
  const sleep = deps.sleepImpl ?? ((ms) => new Promise((r) => setTimeout(r, ms)));
  let locked = false;
  try {
    let attempt = await acquire();
    if (!attempt?.ok) {
      await sleep(150);
      attempt = await acquire();
    }
    locked = Boolean(attempt?.ok);
    if (!locked) {
      try {
        await appendIncident({
          type: "lock-busy",
          lock: "sync",
          detail: String(attempt?.reason ?? "locked"),
        });
      } catch {
        // bookkeeping must never break the guarded operation
      }
    }
    return await fn();
  } finally {
    if (locked) {
      try {
        ...
        // a stale lock is reclaimed later; never surface here
      }
    }
  }
}
