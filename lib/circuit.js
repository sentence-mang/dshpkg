...
// Dangerous state.packages keys: reading/writing them on a plain object
// touches Object.prototype (e.g. state.packages["__proto__"] = {} would
// REPLACE the prototype instead of adding an own property), poisoning every
// object process-wide. Rejected case-insensitively (constructor/Constructor).
const DANGEROUS_KEYS = new Set(["__proto__", "constructor", "prototype"]);

/** Keys that must never be used as a per-package state key. */
...
export function recordCrash(state, entryId, at = Date.now()) {
  ...
  if (isDangerousKey(entryId)) return state; // no prototype pollution
...
export function isOpen(state, entryId, now = Date.now(), opts = DEFAULTS) {
  if (isDangerousKey(entryId)) return false; // never read through the prototype
  const pkg = state?.packages?.[entryId];
...
export function closeCircuit(state, entryId) {
  if (state === null || typeof state !== "object") return null;
  if (typeof entryId !== "string" || entryId.length === 0) return state;
  if (isDangerousKey(entryId)) return state; // never write through the prototype
  const pkg = state.packages?.[entryId];
