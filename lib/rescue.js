...
/**
 * Quote an entry id for a YAML scalar when it is not a plain safe token.
 * Mirrors supervisor.js's yamlSafeId exactly (single-quote wrap + '' escape)
 * so a block written by the supervisor and one written by the CLI serialise
 * identically and can be removed/matched by either side.
 */
export function yamlSafeId(id) {
...
export function removeManagedBlock(patchText, entryId) {
  ...
  const re = new RegExp(
    "# dshpkg:managed:start\\n- id: " +
      escapeRegExp(yamlSafeId(entryId)) +
      "\\n  disabled: true\\n# dshpkg:managed:end\\n?",
    "g"
  );
