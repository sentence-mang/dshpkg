...
> ⚠️ **测试开发阶段（Test / Development Stage）**
>
> 当前版本处于**测试开发阶段**：核心功能已实现并通过单元测试（613 个），但**尚未完成真实环境端到端验证**，接口与行为可能继续调整，**不建议在生产环境使用**。请仅用于测试与开发，遇到问题欢迎反馈。

dshpkg 是 DeepSeek Harness（dsh）的 **apt 风格插件管理器**，目标是 dsh 的**自进化基座**：
